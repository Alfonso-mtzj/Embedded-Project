...
{
    for (int r=0;r<3;r++)
        if (s->board[r][0]!=CELL_EMPTY && s->board[r][0]==s->board[r][1] && s->board[r][1]==s->board[r][2])
            return s->board[r][0];

    for (int c=0;c<3;c++)
        if (s->board[0][c]!=CELL_EMPTY && s->board[0][c]==s->board[1][c] && s->board[1][c]==s->board[2][c])
            return s->board[0][c];

    if (s->board[0][0]!=CELL_EMPTY && s->board[0][0]==s->board[1][1] && s->board[1][1]==s->board[2][2])
        return s->board[0][0];

    if (s->board[0][2]!=CELL_EMPTY && s->board[0][2]==s->board[1][1] && s->board[1][1]==s->board[2][0])
        return s->board[0][2];
